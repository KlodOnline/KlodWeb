...
foreach ($worlds as $world){
	...
	if ($world->demo()=='true') { $line = $line.'<span class="trn">__DEMO__</span>';  }
	else { 
	...
	if ($player->fee_status() OR $world->demo()=='true') {
		...
		if (MAQUETTE_MODE=='docker') {$line = $line.'<form action="'.$world->correct_address(true).'" method="post">';}
		else {$line = $line.'<form action="'.$world->correct_address().'" method="post">';}
