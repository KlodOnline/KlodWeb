...
class app_obj
        ...
        if (is_array($data))
            ...
            foreach ($data AS $data_key => $value)
                ...
                foreach($var_names AS $var_key => $each_name) 
                    {if ($each_name===$data_key) {unset($var_names[$var_key]);} }
                unset($each_name); unset($var_key);
                }
            }
        }
    ...
    public function get_all()    
        ...
        $var_names =  $this->get_var_names();
        $all_vars = array();
        foreach ($var_names AS $each_var_name) { $all_vars[$each_var_name] = $this->$each_var_name(); }
        return $all_vars;
        }
    ...
    public function save()
        ...
        if (isset($object_read)) { $object_manager->update($this);}
        else 
            {
