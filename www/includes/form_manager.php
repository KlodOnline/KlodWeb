...
if(isset($_POST['submit'])) {
  ...
  foreach($_POST as $key => $value) { $data[$key] = filter(filter_var($value,FILTER_SANITIZE_STRING));}

  // var_dump($data);

  if (!array_key_exists("action",$data)) { exit(); }

  if ($data['action']=="register") {

    // Validation of POST data ...
    if(empty($data['username']) or strlen($data['username']) < 4) { $err[] = "__BADSHORTNAME__"; }
    // Validate User Name
    if (!isUserID($data['username'])) { $err[] = "__BADCHARNAME__"; }
    // Validate Email
    if(!isEmail($data['mail'])) { $err[] = "__BADMAIL__"; }
    // Check User Passwords
    if (!checkPwd($data['pwd'],$data['pwd2'])) { $err[] = "__BADPASS__"; }
    ...
    if (empty($err)) {
      ...
      if (count($players)>0) { $err[] = "__DUPLICATENAMEMAIL__"; }
    }
