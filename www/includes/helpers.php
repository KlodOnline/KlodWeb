...
function class_autoloader($classe)
{
    require __DIR__.'/'.$classe.'_class.php';
}
...
function detect_site_string($string_to_inspect)
{
    ...
    foreach ($sites as $each_site) {
        if (stripos($string_to_inspect, $each_site)) {
            return $each_site;
        }
    }
    return '';
...
function that_dir($dir, $from = 'ext')
{
    ...
    $full_dir_path = '';
    if ($from === 'int') {
        $full_dir_path = getcwd().'/'.$dir.'/';
    }
    if ($from === 'ext') {
        $full_dir_path = $_SERVER['HTTP_REFERER'].$dir.'/';
    }

    return $full_dir_path;
}

function fusionne_array($array1 = [], $array2 = [])
{
...
function suffixe($string, $separateur = '_')
{
    ...
    if (substr($string, 0, 1) === $separateur) {
    ...
    if ((sizeof($syllabes) - 1) > 0) {
        $suffixe = end($syllabes);
    }
...
function decode_stock($stock)
{
    ...
    if (is_array($slots)) {
        foreach ($slots as $each_slot) {
            ...
            if (isset($info[1])) {
                $this_prod['volume'] = $info[1];
            } else {
                $this_prod['volume'] = 0;
            }
            $marchandises[] = $this_prod;
        }
        if (is_array($marchandises)) {
            return $marchandises;
        }
    }
}
...
function compile_marchandises($lot_1, $lot_2 = null)
{
    ...
    if (is_array($compiled_prod)) {
        return $compiled_prod;
    }
}
...
function saison($Y, $seed)
{
    ...
    $today = time();
    $i = 0;
    while ($i <= date('j', $today)) {
        if (date("N", mktime(0, 0, 0, date('n', $today), $i, date('Y', $today))) == "1") {
            $week++;
        } $i++;
    }
    ...
    switch ($week) {
        case 1: $saison_nord = $ete;
            $saison_sud = $hiv;
            break;
        case 2: $saison_nord = $aut;
            $saison_sud = $pri;
            break;
        case 3: $saison_nord = $hiv;
            $saison_sud = $ete;
            break;
        case 4: $saison_nord = $pri;
            $saison_sud = $aut;
            break;
    }
...
function numerologis($number)
{
    ...
    $last_number = 0;
    foreach ($chars as $each_char) {
        $last_number = $last_number + $each_char;
    }
...
{
    // Prendre le méridien ?
    if (($X_depart <= $X_arrivee) and ($X_arrivee - $X_depart) <= 150) {
        $meridien = false;
    }
    if (($X_depart <= $X_arrivee) and ($X_arrivee - $X_depart) > 150) {
        $meridien = true;
    }
    if (($X_depart > $X_arrivee) and ($X_depart - $X_arrivee) <= 150) {
        $meridien = false;
    }
    if (($X_depart > $X_arrivee) and ($X_depart - $X_arrivee) > 150) {
        $meridien = true;
    ...
    if (isset($maps)) {
        ...
        if ($meridien) {
            ...
            if ($X_depart > $X_arrivee) {
                $depart = false;
            }	//False = left = depart à guache, donc FirstX= depart
            if ($X_depart <= $X_arrivee) {
                $depart = true;
            }	//true = right = depart à droite, donc FirstX = arrivee (gauche)
            ...
            //Taille de la vue :
            if (!$depart) {
                $X_size = (300 - ($X_depart)) + $X_arrivee + 1;
            } else {
                $X_size = (300 - $X_arrivee) + $X_depart + 1;
            }
            ...
            */
            ...
            for ($y = $Y_first; $y <= $Y_last; $y++) {
                ...
                while ($x <= 300) {
                    ...
                    if (!isset($maps_view[$x_reindex][$y_reindex])) {
                        $maps_view[$x_reindex][$y_reindex] = '';
                    }
        ...
        if (!$meridien) {
            ...
            //Taille de la vue :
            if (!$depart) {
                $X_size = $X_arrivee - ($X_depart - 1);
            } else {
                $X_size = $X_depart - ($X_arrivee - 1);
            }
            $Y_size = $Y_arrivee - $Y_depart + 1;
...
{
    if ($pop > 0 and $pop <= 1000) {
        return 'Hameau';
    } elseif ($pop > 1000 and $pop <= 5000) {
        return 'Colonie';
    } elseif ($pop > 5000 and $pop <= 10000) {
        return 'Village';
    } elseif ($pop > 10000 and $pop <= 20000) {
        return 'Bourg';
    } elseif ($pop > 20000) {
        return 'Ville';
    }
