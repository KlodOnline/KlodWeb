...
class bdd_io
{
    ...
    public function set_my_class($my_class)
    {
        $this ->_my_class = $my_class;
    }
    ...
    public function read_all($limit = 'none')
    {
        $class = $this->my_class();
        if ($limit !== 'none') {
            $request = 'SELECT * FROM '.$class.' WHERE id IS NOT NULL ORDER BY id LIMIT '.$limit;
        } else {
            $request = 'SELECT * FROM '.$class.' WHERE id IS NOT NULL ORDER BY id';
        }
        $result = $this->query($request);
        $objects = $this->to_objects($result, $class);
        if (isset($objects)) {
    ...
    public function insert($object)
    {
        ...
        $to_binds = array_merge([str_repeat($bind_type, count($to_binds))], array_values($to_binds));
        call_user_func_array([&$statement, 'bind_param'], refValues($to_binds));
        $statement->execute();
